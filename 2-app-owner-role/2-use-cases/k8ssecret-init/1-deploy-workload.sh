...
main(){
  ...
  POD_NAME=$(kubectl get pods -n $APP_NAMESPACE_NAME --no-headers | grep app-example-k8s-init | grep Init | awk '{print $1}')
  sleep 1
...
wait_until_ready() {
  ...
  until [[ "$POD_NAME" != "" ]]; do
    sleep 1
    POD_NAME=$(kubectl get pods -n $APP_NAMESPACE_NAME --no-headers | grep Running | grep app-example-k8s-init | awk '{print $1}')
  done
}
