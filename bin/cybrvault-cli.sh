...
showUsage() {
  ...
  echo "    $0 safe_sync_add <safe-name> <sync-user-name>"
...
main() {
  ...
  case $1 in
	...
	cpmName="$4"
	;;
    safe_member_get | safe_member_add | safe_member_delete | safe_admin_add | safe_sync_add  | safe_member_update)
	if [[ $# != 3 ]]; then
	  echo "Incorrect number of arguments."
	  showUsage
	fi
...
function checkDependencies() {
  ...
  if $SELF_HOSTED_PAM; then
    CYBERARK_VAULT_API=$PAM_URL
  else
    CYBERARK_VAULT_API=$PCLOUD_URL
  fi
